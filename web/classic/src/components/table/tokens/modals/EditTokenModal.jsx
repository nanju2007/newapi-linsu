        ...
        if (res.data?.success) {
          const subs = (res.data.data?.subscriptions || [])
            .map((s) => ({
              ...
              title: s?.plan_title || `#${s?.subscription?.id}`,
            }))
            .filter((s) => s.id);
              ...
              <Card className='!rounded-2xl shadow-sm border-0'>
                ...
                <Row gutter={12}>
                  ...
                  {values.subscription_id ? (
                    <Col xs={24} sm={24} md={24} lg={24} xl={24}>
                      <Form.Switch
                        field='allow_balance_fallback'
                        label={t('允许使用余额')}
                        size='default'
                        extraText={t('当订阅额度用完使用余额消耗')}
                      />
                    </Col>
                  ) : null}
                  ...
                  <Col span={24}>
                    <Form.InputNumber
                      field='rpm_limit'
                      label={t('每分钟请求次数限制 (RPM)')}
                      min={0}
                      step={1}
                      extraText={t('限制该令牌每分钟最大请求次数，0 表示不限制')}
                      placeholder={'0'}
                      style={{ width: '100%' }}
                    />
                  </Col>
