...
func Distribute() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if ok {
			...
			// 套餐模型白名单：当令牌绑定订阅且套餐启用模型限制时，校验请求模型是否在允许列表内
			subId := common.GetContextKeyInt(c, constant.ContextKeyTokenSubscriptionId)
			if subId > 0 {
				if planInfo, perr := model.GetSubscriptionPlanInfoByUserSubscriptionId(subId); perr == nil &&
					planInfo != nil && planInfo.ModelLimitsEnabled {
					matchName := ratio_setting.FormatMatchingModelName(modelRequest.Model)
					if _, ok := planInfo.ModelLimits[matchName]; !ok {
						abortWithOpenAiMessage(c, http.StatusForbidden, i18n.T(c, i18n.MsgDistributorTokenModelForbidden, map[string]any{"Model": modelRequest.Model}))
						return
					}
				}
		...
		if channel != nil && c.Writer != nil && c.Writer.Status() < http.StatusBadRequest {
			...
			// 记录本次成功使用的多 Key 索引，供同会话后续请求复用以提升缓存命中率
			if common.GetContextKeyBool(c, constant.ContextKeyChannelIsMultiKey) {
				successChannelID := channel.Id
				if cid := c.GetInt("channel_id"); cid > 0 {
					successChannelID = cid
				}
				service.RecordChannelAffinityKeyIndex(c, successChannelID, common.GetContextKeyInt(c, constant.ContextKeyChannelMultiKeyIndex))
			}
		}
	}
}
...
func SetupContextForSelectedChannel(c *gin.Context, channel *model.Channel, modelName string) *types.NewAPIError {
	...
	// Key 级亲和粘滞：多 Key（号池）渠道下，若同一会话上次成功使用过某个 Key 且该 Key 仍启用，
	// 则优先复用它，以提升上游 prompt cache 命中率；否则维持 GetNextEnabledKey 的随机/轮询结果。
	if channel.ChannelInfo.IsMultiKey {
		if preferredIdx, ok := service.GetPreferredMultiKeyIndexByAffinity(c, channel.Id); ok {
			if stickyKey, enabled := channel.GetMultiKeyByIndexIfEnabled(preferredIdx); enabled {
				key = stickyKey
				index = preferredIdx
			}
		}
