...
const Playground = () => {
  ...
  // API 请求相关
  const { sendRequest, handleImageGeneration, onStopGenerator } = useApiRequest(
    setMessage,
    setDebugData,
    setActiveDebugTab,
    sseSourceRef,
  ...
  function onMessageSend(content, attachment) {
    ...
    // 生图模式：当所选模型匹配生图模型时，自动走生图端点（无需用户手动选择 API 端点）
    if (!customRequestMode && isImageGenerationModel(inputs.model)) {
      const prompt = typeof content === 'string' ? content : getTextContent({ content });
      const imagePayload = {
        model: inputs.model,
        group: inputs.group,
        prompt: prompt || '',
      ...
      setMessage((prevMessage) => {
        ...
        handleImageGeneration(imagePayload);
