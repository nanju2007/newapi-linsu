...
// 邀请返利模式：
//
//	AffRebateModeFixed (0)：维持原有逻辑，注册时一次性按固定额度发放给邀请人。
//	AffRebateModePercentage (1)：被邀请人每次充值成功后，按充值额度的固定百分比返利给邀请人。
const (
	AffRebateModeFixed      = 0
	AffRebateModePercentage = 1
)

// AffRebateMode 当前邀请返利模式，默认固定额度模式（保持向后兼容）。
var AffRebateMode = AffRebateModeFixed

// AffRebatePercentage 按比例返利模式下的返利百分比（0-100），仅在 AffRebateMode=1 时生效。
var AffRebatePercentage = 0.0
