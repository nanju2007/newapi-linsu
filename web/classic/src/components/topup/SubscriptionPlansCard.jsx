                  ...
                  {allSubscriptions.map((sub, subIndex) => {
                    ...
                    return (
                      <div key={subscription?.id || subIndex}>
                        ...
                        {isActive &&
                          Array.isArray(sub.buckets) &&
                          sub.buckets.length > 0 && (
                              ...
                              {sub.buckets.map((b) => {
                                ...
                                return (
                                  ...
                                  >
                                    <span>
                                      {periodLabel}：{t('剩余')}{' '}
                                      {renderQuota(Number(b.remain || 0))} /{' '}
                                      {renderQuota(Number(b.limit || 0))}
                                    </span>
                                    <span>
                                      {t('重置于')} {resetStr}
                                    </span>
          ...
          {plans.length > 0 ? (
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-5 w-full px-1'>
              {plans.map((p, index) => {
                ...
                const validityMonths = Number(plan?.validity_months || 0);
                const validityLabel =
                  validityMonths > 0
                    ? `${t('有效期')}: ${validityMonths} ${t('个月')}`
                    : `${t('有效期')}: ${formatSubscriptionDuration(plan, t)}`;
                const bucketLabels = [];
                if (plan?.daily_enabled)
                  bucketLabels.push({
                    label: `${t('日额度')}: ${renderQuota(Number(plan?.daily_limit || 0))}`,
                  });
                if (plan?.weekly_enabled)
                  bucketLabels.push({
                    label: `${t('周额度')}: ${renderQuota(Number(plan?.weekly_limit || 0))}`,
                  });
                if (plan?.monthly_enabled)
                  bucketLabels.push({
                    label: `${t('月额度')}: ${renderQuota(Number(plan?.monthly_limit || 0))}`,
                  });
  ...
  return (
    <>
      ...
      <SubscriptionPurchaseModal
        ...
        allowBalancePay={selectedPlan?.plan?.allow_balance_pay !== false}
        balanceText={
          selectedPlan?.plan
            ? `${getCurrencyConfig().symbol}${(
                Number(selectedPlan.plan.price_amount || 0) *
                getCurrencyConfig().rate
              ).toFixed(2)}`
            : ''
        }
