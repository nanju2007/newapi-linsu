...
var (
	...
	// multipliersMap 在初始化后只读，热路径中按 Provider 直接取值，
	// 无需加锁（此前的 RWMutex 在高并发下会产生无谓的锁竞争）。
	openAIMultipliers = multipliersMap[OpenAI]
	geminiMultipliers = multipliersMap[Gemini]
	claudeMultipliers = multipliersMap[Claude]

	// mathSymbolSet / urlDelimSet 预构建为集合，避免在热路径中对
	// 每个字符都重建字符串字面量并做 O(n) 线性扫描。
	mathSymbolSet = buildRuneSet("∑∫∂√∞≤≥≠≈±×÷∈∉∋∌⊂⊃⊆⊇∪∩∧∨¬∀∃∄∅∆∇∝∟∠∡∢°′″‴⁺⁻⁼⁽⁾ⁿ₀₁₂₃₄₅₆₇₈₉₊₋₌₍₎²³¹⁴⁵⁶⁷⁸⁹⁰")
	urlDelimSet   = buildRuneSet("/:?&=;#%")
)
...
func getMultipliers(p Provider) multipliers {
	switch p {
	case Gemini:
		return geminiMultipliers
	case Claude:
		return claudeMultipliers
	case OpenAI:
		return openAIMultipliers
	default:
		// 默认兜底 (按 OpenAI 的算)
		return openAIMultipliers
	}
