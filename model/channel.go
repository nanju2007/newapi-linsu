...
// GetMultiKeyByIndexIfEnabled 在多 Key（号池）模式下，按索引返回指定 Key（仅当该 Key 仍为启用状态）。
// 用于渠道亲和的 Key 级粘滞：复用同一会话上次成功使用的上游 Key/账号以提升缓存命中率。
// 非多 Key 渠道、索引越界或该 Key 已被禁用时返回 false，调用方应回退到正常的随机/轮询选择。
func (channel *Channel) GetMultiKeyByIndexIfEnabled(idx int) (string, bool) {
	if !channel.ChannelInfo.IsMultiKey {
		return "", false
	}
	keys := channel.GetKeys()
