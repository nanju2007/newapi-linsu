...
import GeetestCaptcha, { buildGeetestQuery } from '../common/GeetestCaptcha';
import {
  Button,
  Card,
  Checkbox,
...
const LoginForm = () => {
  ...
  const [geetestResult, setGeetestResult] = useState(null);
  ...
  useEffect(() => {
    ...
    if (status?.geetest_enabled) {
      setGeetestEnabled(true);
      setGeetestId(status.geetest_id);
    }
  ...
  async function handleSubmit(e) {
    ...
    if (geetestEnabled && !geetestResult) {
      showInfo(t('请先完成极验验证！'));
      return;
    }
    setSubmitted(true);
    ...
    try {
      if (username && password) {
        const res = await API.post(
          `/api/user/login?turnstile=${turnstileToken}${buildGeetestQuery(geetestResult)}`,
          {
            username,
            password,
          },
  ...
  const renderMarketingPanel = () => {
    ...
    return (
      <div className='relative z-10 w-full max-w-xl'>
        <div className='inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 dark:bg-blue-900/30 mb-8'>
        ...
        <div className='flex items-center gap-3 mb-4'>
          <img src={logo} alt='Logo' className='h-12 rounded-xl' />
          <h1 className='text-4xl xl:text-5xl font-bold text-gray-800 dark:text-gray-100 leading-tight'>
            {systemName}
          </h1>
        </div>

        <p className='text-base text-gray-500 dark:text-gray-400 mb-10 max-w-md'>
          {t('聚合多家 AI 大模型，统一接口接入，稳定高效，助力业务智能化升级。')}
        </p>
        ...
        <div className='space-y-3'>
          {features.map((feature, idx) => (
            <div
              key={idx}
              className='flex items-center gap-3 px-5 py-4 rounded-xl bg-white/70 dark:bg-white/5 backdrop-blur-sm border border-white/40 dark:border-white/10 shadow-sm'
            >
              <span className='flex items-center justify-center w-6 h-6 rounded-full bg-green-100 dark:bg-green-900/40 shrink-0'>
                <svg
  ...
  return (
    <div className='classic-page-fill relative overflow-hidden bg-gray-100 dark:bg-gray-900 flex flex-col lg:flex-row'>
      {/* 背景模糊晕染球 */}
      <div
