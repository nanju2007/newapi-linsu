...
export const processRawData = (
  ...
  const result = {
    ...
    timeCacheTokensMap: new Map(),
    timeUseTimeMap: new Map(),
  };
  ...
  data.forEach((item) => {
    ...
    result.totalCacheTokens += item.cache_tokens || 0;
    result.totalUseTime += item.use_time || 0;
    ...
    // 缓存 token 与响应时间按时间聚合
    result.timeCacheTokensMap.set(
      timeKey,
      (result.timeCacheTokensMap.get(timeKey) || 0) + (item.cache_tokens || 0),
    );
    result.timeUseTimeMap.set(
      timeKey,
      (result.timeUseTimeMap.get(timeKey) || 0) + (item.use_time || 0),
    );
  });
...
export const calculateTrendData = (
  ...
  const cacheTokensTrend = timePoints.map(
    (time) => timeCacheTokensMap.get(time) || 0,
  );
  // 平均响应（秒）= 该时间段总响应耗时 / 该时间段请求次数
  const avgResponseTrend = timePoints.map((time) => {
    const count = timeCountMap.get(time) || 0;
    const useTime = timeUseTimeMap.get(time) || 0;
    return count > 0 ? useTime / count : 0;
  });
  ...
  return {
    ...
    tokens: tokensTrend,
    cacheTokens: cacheTokensTrend,
    avgResponse: avgResponseTrend,
    rpm: rpmTrend,
    tpm: tpmTrend,
  };
};
