...
// applySubscriptionPriceRatio 在使用订阅计费时，用套餐倍率「替代」分组倍率：
// 模型实际价格 = 模型价格 * 套餐倍率（而非分组倍率）。
// 传入的 quota 已包含分组倍率，这里先除回分组倍率再乘套餐倍率。
// 仅缩放最终结算额度，预扣费保持原值，由结算 delta 自动对账。
// 注意：仅在订阅资金来源时生效；若订阅额度耗尽回退到钱包，BillingSource 为
// wallet，则不会进入此分支，继续使用分组倍率。
func applySubscriptionPriceRatio(relayInfo *relaycommon.RelayInfo, quota int) int {
	...
	ratio := relayInfo.SubscriptionPriceRatio
	if ratio <= 0 {
		ratio = 1
	}
	groupRatio := 1.0
	...
	scaled := decimal.NewFromInt(int64(quota)).
...
func PostWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelName string,
	...
	quota = applySubscriptionPriceRatio(relayInfo, quota)
