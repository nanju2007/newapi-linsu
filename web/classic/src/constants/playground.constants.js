...
// ========== 生图模型匹配 ==========
// 匹配到以下（不区分大小写、子串匹配）模型名时，自动走生图端点，无需用户手动选择 API 端点。
export const IMAGE_MODEL_PATTERNS = ['gpt-image'];
...
export const isImageGenerationModel = (modelName) => {
  ...
  return IMAGE_MODEL_PATTERNS.some((pattern) => lower.includes(pattern));
};
