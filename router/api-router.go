...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		{
			userRoute.POST("/register", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, middleware.TurnstileCheck(), middleware.GeetestCheck(), controller.Register)
			userRoute.POST("/login", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, middleware.TurnstileCheck(), middleware.GeetestCheck(), controller.Login)
			...
			selfRoute.Use(middleware.UserAuth())
			{
				...
				selfRoute.POST("/withdrawal", middleware.CriticalRateLimit(), controller.CreateWithdrawal)
				selfRoute.GET("/withdrawal/self", controller.GetUserWithdrawals)
			...
			adminRoute.Use(middleware.AdminAuth())
			{
				...
				// 邀请返利提现管理
				adminRoute.GET("/withdrawal", controller.GetAllWithdrawals)
				adminRoute.POST("/withdrawal/approve", controller.ApproveWithdrawal)
				adminRoute.POST("/withdrawal/reject", controller.RejectWithdrawal)
			}
		}
		...
		helpRoute.Use(middleware.UserAuth())
		{
			helpRoute.GET("/", controller.GetHelpDocs)
			helpRoute.GET("/categories", controller.GetHelpCategories)
			helpRoute.GET("/:id", controller.GetHelpDocDetail)
		}

		// 帮助中心（管理员侧：CRUD + 图片上传）。独立前缀避免与 /help/:id 通配冲突。
		helpAdminRoute := apiRouter.Group("/help_admin")
		helpAdminRoute.Use(middleware.AdminAuth())
		{
			helpAdminRoute.GET("/all", controller.AdminGetHelpDocs)
			helpAdminRoute.GET("/:id", controller.AdminGetHelpDocDetail)
			helpAdminRoute.POST("/", controller.AdminCreateHelpDoc)
			helpAdminRoute.PUT("/:id", controller.AdminUpdateHelpDoc)
			helpAdminRoute.DELETE("/:id", controller.AdminDeleteHelpDoc)
			helpAdminRoute.POST("/upload", middleware.CriticalRateLimit(), controller.AdminUploadHelpImage)
		}
		...
		// 日志导出（Excel/PDF）：小数据量同步返回文件，大数据量转异步任务
		logRoute.GET("/export", middleware.UserAuth(), controller.ExportLogs)
		logRoute.GET("/export/status", middleware.UserAuth(), controller.GetLogExportStatus)
		logRoute.GET("/export/download", middleware.UserAuth(), controller.DownloadLogExport)
