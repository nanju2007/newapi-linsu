...
const (
	...
	channelAffinityKeyCacheNamespace        = "new-api:channel_affinity_key:v1"
	channelAffinityUsageCacheStatsNamespace = "new-api:channel_affinity_usage_cache_stats:v1"

	ginKeyChannelAffinityStickyKeyIndex = "channel_affinity_sticky_key_index"
)
...
var (
	...
	// channelAffinityKeyCache 记录"亲和键 -> 上次成功使用的多 Key（号池）索引"，
	// 用于在亲和有效期内复用同一上游 Key/账号以提升 prompt cache 命中率。
	channelAffinityKeyCacheOnce sync.Once
	channelAffinityKeyCache     *cachex.HybridCache[int]
...
func getChannelAffinityKeyCache() *cachex.HybridCache[int] {
	channelAffinityKeyCacheOnce.Do(func() {
		setting := operation_setting.GetChannelAffinitySetting()
		capacity := setting.MaxEntries
		if capacity <= 0 {
			capacity = 100_000
		...
		channelAffinityKeyCache = cachex.NewHybridCache[int](cachex.HybridCacheConfig[int]{
			Namespace: cachex.Namespace(channelAffinityKeyCacheNamespace),
			Redis:     common.RDB,
			RedisEnabled: func() bool {
				return common.RedisEnabled && common.RDB != nil
			},
	...
	return channelAffinityKeyCache
}

// buildChannelAffinityKeyCacheKey 组合"渠道 + 亲和键后缀"作为 Key 粘滞缓存键，
// 确保不同渠道、不同亲和键之间互不串用。
func buildChannelAffinityKeyCacheKey(channelID int, cacheKeySuffix string) string {
	cacheKeySuffix = strings.TrimSpace(cacheKeySuffix)
	if channelID <= 0 || cacheKeySuffix == "" {
		return ""
	}
	return strconv.Itoa(channelID) + ":" + cacheKeySuffix
}

// GetPreferredMultiKeyIndexByAffinity 在亲和有效期内返回上次成功使用的多 Key 索引。
func GetPreferredMultiKeyIndexByAffinity(c *gin.Context, channelID int) (int, bool) {
	setting := operation_setting.GetChannelAffinitySetting()
	if setting == nil || !setting.Enabled || !setting.StickyKey {
		return 0, false
	}
	...
	keyCacheKey := buildChannelAffinityKeyCacheKey(channelID, meta.CacheKeySuffix)
	if keyCacheKey == "" {
		return 0, false
	}
	cache := getChannelAffinityKeyCache()
	idx, found, err := cache.Get(keyCacheKey)
	if err != nil || !found {
		return 0, false
	}
	return idx, true
...
// RecordChannelAffinityKeyIndex 记录本次成功使用的多 Key 索引，供后续同会话请求复用。
func RecordChannelAffinityKeyIndex(c *gin.Context, channelID int, keyIndex int) {
	...
	cache := getChannelAffinityKeyCache()
	if err := cache.SetWithTTL(keyCacheKey, keyIndex, time.Duration(ttlSeconds)*time.Second); err != nil {
		common.SysError(fmt.Sprintf("channel affinity key cache set failed: key=%s, err=%v", keyCacheKey, err))
	}
}
