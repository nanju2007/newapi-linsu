...
type ChannelAffinitySetting struct {
	...
	// StickyKey 控制多 Key（号池）渠道下是否在亲和有效期内复用同一个上游 Key/账号。
	// 开启后，同一会话（同一亲和键）的后续请求会优先命中上次成功使用的 Key，
	// 以提升上游 prompt cache 命中率；该 Key 被禁用或失效时自动回退到正常的随机/轮询选择。
	StickyKey         bool                  `json:"sticky_key"`
