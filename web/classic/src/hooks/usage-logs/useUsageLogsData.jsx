...
export const useLogsData = () => {
  ...
  const setLogsFormat = (logs) => {
    ...
    for (let i = 0; i < logs.length; i++) {
      ...
      if (logs[i].type === 2) {
        ...
        const isSubscriptionBilling = other?.billing_source === 'subscription';
        const subscriptionPriceRatio =
          Number(other?.subscription_price_ratio) > 0
            ? Number(other.subscription_price_ratio)
            : 1;
        let content = '';
        if (!isViolationFeeLog && other?.billing_mode !== 'tiered_expr') {
          ...
          if (isSubscriptionBilling) {
            // 订阅计费：用套餐独立倍率替代分组倍率参与计算与展示
            logOpts.group_ratio = subscriptionPriceRatio;
            logOpts.user_group_ratio = -1;
            logOpts.ratio_label_override = t('订阅倍率');
          }
  ...
  // 导出日志：构造与当前筛选一致的查询，调用后端导出接口。
  // 小数据量后端直接返回文件（blob）；大数据量返回 job 信息，前端轮询后再下载。
  const exportLogs = async (format = 'xlsx') => {
    ...
    let qs = `format=${format}&type=${currentLogType}&token_name=${token_name}&model_name=${model_name}&start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}&group=${group}&request_id=${request_id}`;
    if (isAdminUser) {
      qs += `&username=${username}&channel=${channel}`;
    }
    const baseUrl = encodeURI(`/api/log/export?${qs}`);

    try {
      // 先以 blob 请求；若返回 JSON（异步任务）则转轮询
      ...
      if (contentType.includes('application/json')) {
        ...
        if (!json.success) {
          showError(json.message || t('导出失败'));
          return;
        }
        const jobId = json.data?.job_id;
        showSuccess(t('数据量较大，正在后台生成，请稍候…'));
        await pollAndDownloadExport(jobId, format);
        return;
      }
      triggerBlobDownload(res.data, format);
      showSuccess(t('导出成功'));
    } catch (e) {
      showError(t('导出失败'));
    }
  ...
  const triggerBlobDownload = (blob, format) => {
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `logs.${format === 'pdf' ? 'pdf' : 'xlsx'}`;
  ...
  const pollAndDownloadExport = async (jobId, format) => {
    if (!jobId) return;
    const maxTries = 600; // 最多轮询约 10 分钟（每 1s）
    for (let i = 0; i < maxTries; i++) {
      await new Promise((r) => setTimeout(r, 1000));
      try {
        const res = await API.get(
          `/api/log/export/status?job_id=${encodeURIComponent(jobId)}`,
        );
        const { success, data } = res.data;
        if (!success) continue;
        if (data.status === 'done') {
          const dl = await API.get(
            `/api/log/export/download?job_id=${encodeURIComponent(jobId)}`,
            { responseType: 'blob' },
          );
          triggerBlobDownload(dl.data, format);
          showSuccess(t('导出成功'));
          return;
        }
        if (data.status === 'failed') {
          showError(data.error || t('导出失败'));
          return;
        }
      } catch (e) {
        // 单次轮询失败忽略，继续重试
