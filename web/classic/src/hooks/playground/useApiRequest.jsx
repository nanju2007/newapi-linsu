...
export const useApiRequest = (
  ...
  // 生图请求（POST /pg/images/generations）
  const handleImageGeneration = useCallback(
    async (payload) => {
      ...
      try {
        ...
        // 将返回的图片转换为消息内容（image_url 数组），复用现有图片渲染
        ...
        const revisedPrompt = data?.data?.find(
          (item) => item.revised_prompt,
        )?.revised_prompt;

        const content = [...imageItems];
        if (revisedPrompt) {
    ...
    [setDebugData, setActiveDebugTab, setMessage, t, saveMessages],
  );
