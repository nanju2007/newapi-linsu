...
type RelayInfo struct {
	...
	// DeferFirstResponseToContent 为 true 时，StreamScannerHandler 不在收到任意 chunk 时设置首字时间，
	// 而是交由各 handler 在解析出「第一个文字类型响应」时调用 SetFirstResponseTime，
	// 使首字（TTFT）统计更精确。默认 false（保持旧行为，收到任意响应即算首字）。
	DeferFirstResponseToContent bool
	...
	SubscriptionId int
	// TokenSubscriptionId binds the request's token to a specific user subscription
	// (0 = not bound). When > 0, billing is forced onto that subscription.
	TokenSubscriptionId int
	// TokenAllowBalanceFallback allows falling back to wallet balance once the
	// bound subscription's quota is exhausted.
	TokenAllowBalanceFallback bool
	...
	// SubscriptionPriceRatio is the plan's billing multiplier applied to the final
	// quota when BillingSource == "subscription" (normalized: <=0 means no scaling).
	SubscriptionPriceRatio float64
...
func genBaseRelayInfo(c *gin.Context, request dto.Request) *RelayInfo {
	...
	info := &RelayInfo{
		...
		TokenSubscriptionId:       c.GetInt("token_subscription_id"),
		TokenAllowBalanceFallback: c.GetBool("token_allow_balance_fallback"),
