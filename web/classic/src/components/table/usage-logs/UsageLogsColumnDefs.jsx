...
function getStreamEndSideText(side, t) {
  switch (side) {
    case 'client':
      return t('客户端主动中断');
    case 'upstream':
...
function buildStreamStatusTooltip(ss, t) {
  if (!ss) return null;
  const lines = [t('流状态') + '：' + t('异常')];
  const sideText = getStreamEndSideText(ss.end_side, t);
  if (sideText) {
    lines.push(`${t('中断方')}：${sideText}`);
  }
  if (ss.end_desc) {
    lines.push(ss.end_desc);
  }
  lines.push(`${t('原因标识')}：${ss.end_reason || 'unknown'}`);
  if (ss.error_count > 0) {
    lines.push(`${t('软错误')}: ${ss.error_count}`);
  }
  if (ss.end_error) {
    lines.push(`${t('错误详情')}：${ss.end_error}`);
  }
  return (
    <div style={{ lineHeight: 1.6, display: 'flex', flexDirection: 'column' }}>
      {lines.map((line, i) => (
...
// 将秒数格式化为可读时长：>=60s 显示为分钟（60s => 1分钟，90s => 1.5分钟），否则显示秒
...
function renderUseTime(type, t) {
  ...
  const label = formatDuration(time, t);
  if (time < 101) {
    return (
...
// 输出吞吐量提示：补全 tokens / 用时(秒) => tokens/s
function renderThroughput(record, t) {
  const useTime = parseFloat(record.use_time);
  const completionTokens = parseInt(record.completion_tokens);
  if (!useTime || useTime <= 0 || !completionTokens || completionTokens <= 0) {
    return null;
  }
  const tps = completionTokens / useTime;
  const tpsText = tps >= 100 ? Math.round(tps) : parseFloat(tps.toFixed(1));
  let color = 'green';
  if (tps < 10) {
    color = 'red';
  } else if (tps < 30) {
  ...
  return (
    <Tooltip content={t('输出吞吐量（补全 tokens / 用时）')}>
  ...
  if (other?.billing_source === 'subscription') {
    const subscriptionPriceRatio =
      Number(other?.subscription_price_ratio) > 0
        ...
        : 1;
    summaryOpts.group_ratio = subscriptionPriceRatio;
    summaryOpts.user_group_ratio = -1;
    summaryOpts.ratio_label_override = t('订阅倍率');
  }
