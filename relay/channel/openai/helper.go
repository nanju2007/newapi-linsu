...
// extractStreamUsage 尝试从单个 SSE chunk 中解析有效 usage。
// 用于兜底捕获位于非最后一个 chunk 的 usage（部分上游/号池会将 usage 放在
// finish_reason chunk 之后、[DONE] 之前的独立 chunk，或 usage chunk 之后还跟随
// 空 choices chunk），避免仅解析最后一个 chunk 导致 usage 丢失、输入输出显示为 0。
...
// streamChunkHasContent 判断一个 SSE chunk 是否含「文字类型」内容
// （正文 content、推理 reasoning、工具调用名称/参数）。用于精确判定首字时间，
// 避免仅含 role 的空 delta 或仅含 usage 的 chunk 被误判为首字。
func streamChunkHasContent(data string) bool {
	if data == "" {
		return false
	}
	var streamResponse dto.ChatCompletionsStreamResponse
	...
	if service.ValidUsage(lastStreamResponse.Usage) {
		...
		*usage = lastStreamResponse.Usage
		if !info.ShouldIncludeUsage {
			// 客户端未请求 usage（usage 通常是 newapi 通过 ForceStreamOption 强制开启
			// 才由上游返回的）。此时若末尾 chunk 仍携带其它内容（finish_reason、正文等），
			// 不能整体丢弃，否则客户端会收不到结束块（表现为「上游有返回但 newapi 不下发」）；
			// 应剥离 usage 字段后照常下发。仅当该 chunk 是纯 usage（无 choices）时才不下发。
			if len(lastStreamResponse.Choices) > 0 {
				stripped := lastStreamResponse
				stripped.Usage = nil
				if strippedBytes, err := common.Marshal(stripped); err == nil {
					*lastStreamData = string(strippedBytes)
				}
