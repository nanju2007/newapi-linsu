  ...
  const popoverContent = (
    <div style={{ width: 260 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        ...
        {plan?.daily_enabled && (
          <>
            <Text type='tertiary'>{t('日额度')}</Text>
            <Text>{renderQuota(Number(plan?.daily_limit || 0))}</Text>
          </>
        )}
        {plan?.weekly_enabled && (
          <>
            <Text type='tertiary'>{t('周额度')}</Text>
            <Text>{renderQuota(Number(plan?.weekly_limit || 0))}</Text>
          </>
        )}
        {plan?.monthly_enabled && (
          <>
            <Text type='tertiary'>{t('月额度')}</Text>
            <Text>{renderQuota(Number(plan?.monthly_limit || 0))}</Text>
          </>
        )}
        ...
        <Text>
          {Number(plan?.validity_months || 0) > 0
            ? `${plan.validity_months} ${t('个月')}`
...
const renderDuration = (text, record, t) => {
  const months = Number(record?.plan?.validity_months || 0);
  return (
    <Text type='secondary'>
      {months > 0 ? `${months} ${t('个月')}` : formatDuration(record?.plan, t)}
    </Text>
  );
};
