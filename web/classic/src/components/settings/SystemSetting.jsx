...
const SystemSetting = () => {
  ...
  const submitGeetest = async () => {
    const options = [];

    if (originInputs['GeetestId'] !== inputs.GeetestId) {
      options.push({ key: 'GeetestId', value: inputs.GeetestId });
    }
    if (
      originInputs['GeetestKey'] !== inputs.GeetestKey &&
      inputs.GeetestKey !== ''
    ) {
      options.push({
        key: 'GeetestKey',
        value: inputs.GeetestKey,
      });
    }
        ...
        >
          {({ formState, values, formApi }) => (
            ...
            >
                ...
                <Form.Section text={t('配置登录注册')}>
                  ...
                  >
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      ...
                      <Form.Checkbox
                        field='GeetestEnabled'
                        noLabel
                        onChange={(e) =>
                          handleCheckboxChange('GeetestEnabled', e)
                        }
                      >
                        {t('允许极验用户校验')}
                      </Form.Checkbox>
              ...
              <Card>
                <Form.Section text={t('配置极验验证码')}>
                  <Text>
                    {t('用以支持极验行为验证 4.0，需在极验后台申请验证 ID 与 KEY')}
                  </Text>
                  <Row
                    gutter={{ xs: 8, sm: 16, md: 24, lg: 24, xl: 24, xxl: 24 }}
                  >
