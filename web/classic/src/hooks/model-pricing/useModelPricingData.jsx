...
export const useModelPricingData = () => {
  ...
  const getModelUptimeMonitor = (model) => {
    if (!model?.model_name || uptimeMonitors.length === 0) return null;
    const modelName = model.model_name.toLowerCase();
    return (
      uptimeMonitors.find(
        (item) => item?.name && item.name.toLowerCase() === modelName,
      ) ||
      uptimeMonitors.find(
        (item) => item?.name && item.name.toLowerCase().includes(modelName),
      ) ||
      null
    );
  };
