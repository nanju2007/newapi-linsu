...
export const useDashboardCharts = (
    ...
    tooltip: {
      mark: {
        content: [
          {
            key: (datum) => datum['Type'],
            value: (datum) => renderNumber(datum['Value'] || 0),
          },
        ],
      },
      dimension: {
  ...
  const updateChartData = useCallback(
    (data) => {
      ...
      // ===== Token 使用趋势（总Token + 缓存读取Token） =====
      const tokenTrendData = [];
      timePoints.forEach((time) => {
        tokenTrendData.push({
          Time: time,
          Type: t('总 Token'),
          Value: timeTokensMap.get(time) || 0,
        });
        tokenTrendData.push({
          Time: time,
          Type: t('缓存读取'),
          Value: timeCacheTokensMap.get(time) || 0,
        });
      });
      setSpecTokenTrend((prev) => ({
        ...prev,
        data: [{ id: 'tokenTrendData', values: tokenTrendData }],
        point: {
          visible: timePoints.length <= 1,
        },
        title: {
          ...prev.title,
