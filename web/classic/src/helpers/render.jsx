 ...
 */
function getEffectiveRatio(groupRatio, user_group_ratio, ratioLabelOverride) {
  if (ratioLabelOverride) {
    return {
      ratio: groupRatio,
      label: ratioLabelOverride,
...
function getGroupRatioText(groupRatio, user_group_ratio, ratioLabelOverride) {
...
function renderPriceSimpleCore({
  ...
  if (outputMode === 'segments') {
    const segments = [
      {
        tone: 'primary',
        text: getGroupRatioText(groupRatio, user_group_ratio, ratioLabelOverride),
      },
    ];
  ...
  if (modelPrice !== -1) {
    if (isPriceDisplayMode(displayMode, modelPrice)) {
      return joinBillingSummary([
        ...
        getGroupRatioText(groupRatio, user_group_ratio, ratioLabelOverride),
      ]);
    }
  ...
  if (isPriceDisplayMode(displayMode, modelPrice)) {
    ...
    if (modelPrice !== -1) {
      ...
      parts.push(getGroupRatioText(groupRatio, user_group_ratio, ratioLabelOverride));
...
export function renderModelPrice(opts) {
  const {
    ...
    displayMode = 'price',
    ratio_label_override: ratioLabelOverride,
  } = opts;
  const { ratio: effectiveGroupRatio, label: ratioLabel } = getEffectiveRatio(
      _groupRatio,
      user_group_ratio,
