...
export default function GeneralSettings(props) {
  ...
  const [inputs, setInputs] = useState({
    ...
    'general_setting.theme_preset': 'default',
    'general_setting.theme_primary_color': '',
  });
  ...
  useEffect(() => {
    ...
    // 主题相关字段始终回填，避免后端未持久化时键缺失导致 compareObjects 无法检测变更
    currentInputs['general_setting.theme_preset'] =
      props.options['general_setting.theme_preset'] !== undefined
        ? props.options['general_setting.theme_preset']
        : 'default';
    currentInputs['general_setting.theme_primary_color'] =
      props.options['general_setting.theme_primary_color'] !== undefined
        ? props.options['general_setting.theme_primary_color']
        : '';
    ...
    <>
      <Spin spinning={loading}>
        ...
        >
          ...
          <Form.Section text={t('网站主题颜色')}>
            <Row gutter={16}>
              <Col xs={24} sm={12} md={8} lg={8} xl={8}>
                <Form.Select
                  field='general_setting.theme_preset'
                  label={t('主题色调')}
                  extraText={t('内置预设色调，default 为 newapi 默认经典白底')}
                  style={{ width: '100%' }}
                  onChange={(val) => {
                    handleFieldChange('general_setting.theme_preset')(val);
                    // 实时预览（自定义色优先；为空则用预设）
                    applyThemeColor(
                      val,
                      inputs['general_setting.theme_primary_color'],
                    );
                  }}
                >
                  {Object.entries(THEME_PRESETS).map(([key, conf]) => (
                      ...
                      <div className='flex items-center gap-2'>
                        <span
                          style={{
                            ...
                            background: conf.primary || '#2563eb',
                          }}
                        />
                        {t(conf.label)}
                      </div>
              ...
              <Col xs={24} sm={12} md={8} lg={8} xl={8}>
                <Form.Slot label={t('自定义主色（可选）')}>
                  <div className='flex items-center gap-2'>
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        applyThemeColor(
                          inputs['general_setting.theme_preset'],
                          val,
                        );
                      }}
                      style={{
                    ...
                    <Input
                      placeholder='#2563eb'
                      value={inputs['general_setting.theme_primary_color'] || ''}
                      onChange={(val) => {
                        handleFieldChange(
                          'general_setting.theme_primary_color',
                        )(val);
                  ...
                  <Text
                    ...
                    style={{ marginTop: 4, display: 'block' }}
                  >
                    {t('填写 HEX 颜色（如 #2563eb）将覆盖预设主色；留空则使用所选预设')}
                  </Text>
                </Form.Slot>
              </Col>
            </Row>
            <Row gutter={16}>
              <Col span={24}>
                <Text type='tertiary' size='small'>
                  {t('提示：保存后对全站生效。预览仅在当前页面临时显示，刷新或保存后以最终配置为准')}
                </Text>
              </Col>
            </Row>
